...
if __name__ == "__main__":
    ...
    # load 20 model
    model_SR20 = RDSR_20()
    model_SR20.load_state_dict(torch.load("model.20.pt", map_location=device)["model"])
    ...
    for x_10m, x_20m in zip(
        ...
        with torch.no_grad():
            ys.append(d_norm_t_20m(model_SR20(norm_t_10m(x_10m), norm_t_20m(x_20m))))
    ...
    # load 60 model
    model_SR60 = RDSR_60()
    model_SR60.load_state_dict(torch.load("model.60.pt", map_location=device)["model"])
