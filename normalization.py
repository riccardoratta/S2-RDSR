...
def dataset_std_mean(dataLoader: DataLoader, resolutions: List[str]):
    ...
    n = 0; n_max = len(dataLoader); limit = 0
    for value, _ in dataLoader:

        print(f'\r{n / n_max * 100:.2f}%', end='', flush=True)

        for i, r in enumerate(resolutions):
            std[r] += batch_band(value[i], torch.std); mean[r] += batch_band(value[i], torch.mean)
...
if __name__ == '__main__':
    ...
    if '20' in args.type:
        ...
        with open(p.join(args.path, 'SR20', 'norm.json'), 'w') as f:
            f.write(json.dumps({'mean': mean, 'std': std}, default=tensor_JSONEncoder, indent=2))
