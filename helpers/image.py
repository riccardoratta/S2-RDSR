...
class Image:
    ...
    @property
    def tensor(self):
        return F.to_tensor(self.image).type(torch.int16)


def downscale(scale: int):
    """
    Get the downscale transform, this will apply a Gaussian blur and downscale with an average pool.
    """
    sigma = 1 / scale
    kernel_size = 2 * (round(4 * sigma)) + 1
    return T.Compose(
        [T.GaussianBlur(kernel_size, sigma), nn.AvgPool2d(kernel_size=scale)]
    )
