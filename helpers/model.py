...
def model_eval(
    model: nn.Module, dataLoader: DataLoader, eval_fns: Union[Callable, List[Callable]]
):
    model.eval()
...
def at_epoch(
    ...
    for epoch in range(*epochs):
        step_v = model_step(
            model, dataLoader, loss_fn, optimizer, scheduler, gradient_clipping
        )

        eval_v = model_eval(model, eval_dataLoader, eval_fn)
